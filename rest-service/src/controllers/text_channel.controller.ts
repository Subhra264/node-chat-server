...
export default {
  ...
  ): Promise<{ name: string; reference: any }> => {
    try {
      const validatedTextChannel: TextChannelSchema =
        await TextChannel_Joi.validateAsync(req.body);
      ...
      user.groups.forEach((group: any) => {
        ...
        if (
          JSON.stringify(group._id) === `"${validatedTextChannel.parentGroup}"`
        ) {
          isAllowed = true;
        }
      });
      ...
      const newTextChannel: TextChannelDocument = await (
        new TextChannel(validatedTextChannel) as TextChannelDocument
      ).save();
      ...
      const parentGroup: GroupDocument = await Group.findById(
        validatedTextChannel.parentGroup,
      ).exec();
  ...
  saveMessage: async (req: AuthenticatedRequest): Promise<void> => {
    try {
      const user: AuthenticatedCachedUser = req.user as AuthenticatedCachedUser;
      const { channelId, message, groupId } =
        await MessageReqSchema_Joi.validateAsync(req.body);
